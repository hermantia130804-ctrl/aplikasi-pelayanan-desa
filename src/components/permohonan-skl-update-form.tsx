...
import { updatePermohonanSKLAction } from "@/lib/server/actions/permohonan-skl";
import { updatePermohonanSKLSchema } from "@/lib/validators/permohonan-skl";
...
type PermohonanSKLUpdateFormProps = {
  permohonanSKL: PermohonanSKL;
};

export const PermohonanSKLUpdateForm = ({ permohonanSKL }: PermohonanSKLUpdateFormProps) => {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const form = useForm<z.infer<typeof updatePermohonanSKLSchema>>({
    resolver: zodResolver(updatePermohonanSKLSchema),
    defaultValues: {
      nama: permohonanSKL.nama || "",
      tempatLahir: permohonanSKL.tempatLahir || "",
      tanggalLahir: permohonanSKL.tanggalLahir || new Date(),
      jenisKelamin: permohonanSKL.jenisKelamin || "LAKI_LAKI",
      alamat: permohonanSKL.alamat || "",
      
      // Data Ayah
      namaAyah: permohonanSKL.namaAyah || "",
      nikAyah: permohonanSKL.nikAyah || "",
      pekerjaanAyah: permohonanSKL.pekerjaanAyah || "",
      tempatLahirAyah: permohonanSKL.tempatLahirAyah || "",
      tanggalLahirAyah: permohonanSKL.tanggalLahirAyah || new Date(),
      agamaAyah: permohonanSKL.agamaAyah || "ISLAM",
      
      // Data Ibu
      namaIbu: permohonanSKL.namaIbu || "",
      nikIbu: permohonanSKL.nikIbu || "",
      pekerjaanIbu: permohonanSKL.pekerjaanIbu || "",
      tempatLahirIbu: permohonanSKL.tempatLahirIbu || "",
      tanggalLahirIbu: permohonanSKL.tanggalLahirIbu || new Date(),
      agamaIbu: permohonanSKL.agamaIbu || "ISLAM",
      
      // Dokumen
      dokumenKK: permohonanSKL.dokumenKK || "",
      dokumenPengantar: permohonanSKL.dokumenPengantar || "",
      dokumenSuratLahir: permohonanSKL.dokumenSuratLahir || "",
      catatan: permohonanSKL.catatan || "",
    },
  });

  const onSubmit = async (values: z.infer<typeof updatePermohonanSKLSchema>) => {
    try {
      setLoading(true);
      const response = await updatePermohonanSKLAction(permohonanSKL.permohonanSKLId, values);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Edit formulir permohonan Surat Keterangan Lahir atas nama {permohonanSKL.nama}.
          </p>
        </div>
      </div>
