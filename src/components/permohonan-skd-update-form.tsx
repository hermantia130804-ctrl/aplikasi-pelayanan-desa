...
import { Agama, JenisKelamin, PermohonanSKD, WargaNegara } from "@/generated/prisma";
import { cn } from "@/lib/utils";
import { updatePermohonanSKDSchema } from "@/lib/validators/permohonan-skd";
import { updatePermohonanSKDAction } from "@/lib/server/actions/permohonan-skd";
...
type UpdatePermohonanSKDFormData = z.infer<typeof updatePermohonanSKDSchema>;
...
interface PermohonanSKDUpdateFormProps {
  permohonan: PermohonanSKDWithUser;
}

export function PermohonanSKDUpdateForm({ permohonan }: PermohonanSKDUpdateFormProps) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);

  const form = useForm<UpdatePermohonanSKDFormData>({
    resolver: zodResolver(updatePermohonanSKDSchema),
    defaultValues: {
      nik: permohonan.nik,
      nama: permohonan.nama,
      alamatKTP: permohonan.alamatKTP,
      alamatDomisili: permohonan.alamatDomisili,
      ...
      wargaNegara: permohonan.wargaNegara,
  ...
  const onSubmit = async (data: UpdatePermohonanSKDFormData) => {
    try {
      setIsLoading(true);
      ...
      const result = await updatePermohonanSKDAction(permohonan.permohonanSKDId, formData);
      ...
      toast.error("Gagal mengupdate permohonan SKD");
