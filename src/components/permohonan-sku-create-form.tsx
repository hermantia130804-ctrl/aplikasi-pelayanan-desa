...
export function PermohonanSKUCreateForm() {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);

  const form = useForm<TCreatePermohonanSKUSchema>({
    resolver: zodResolver(createPermohonanSKUSchema),
    defaultValues: {
      nik: "",
      nama: "",
      tempatLahir: "",
      ...
      alamat: "",
      jenisUsaha: "",
      tahunBerdiriUsaha: new Date().getFullYear(),
      lokasiUsaha: "",
      catatan: "",
      dokumenKK: "",
      dokumenKTP: "",
...
<FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Dokumen Usaha (PDF)</FormLabel>
                  <FormControl>
                    <div className="relative">
                      <FileUploadField
                        value={field.value || ""}
                        onChange={field.onChange}
                        accept=".pdf"
                        placeholder="Klik untuk memilih dokumen usaha (PDF)"
                        bucketName="docs"
                      />
                      {field.value && (
                        <span className="absolute top-2 right-2 text-green-600 flex items-center gap-1 text-xs font-medium">
